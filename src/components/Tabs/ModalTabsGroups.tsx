...
export default function ModalTabsGroups(cn:any) {
  ...
  const onUpdateReplaceGroup = async (FormData:any) => {
    ...
    if (token) {
      const result = await dataGroups.update(cn['cn'].cn,updateBody,token);
      setData(result)
    } else {
  ...
  const onUpdateAddGroup = async (FormData:any) => {
    let updateBody = FormData
...
const onUpdateDelGroup = async (selectedValues:any) => {
        ...
        <CardBody>
          ...
          >
            <Tab key="add" title="Añadir campos">
              <form className="flex flex-col gap-4" onSubmit={ handleSubmit(onUpdateAddGroup) } noValidate>
            ...
            <form className="flex flex-col gap-4 h-[500px]" onSubmit={ handleSubmit(onUpdateReplaceGroup) } noValidate>
            ...
            <Tab className="flex flex-col gap-4" key="delete" title="Eliminar campos">
                ...
                <Button color="danger" onClick={() => onUpdateDelGroup(selectedValues)}>
                    Eliminar campos
                </Button>
            </Tab>
          </Tabs>

...
import createOusSchema from "@/lib/ou.yup";
import { appConfig } from "@/config/appConfig";

export const ModalFormOusPost = () => {
  const [data, setData] = useState<any>();
  const { isOpen, onOpen, onOpenChange } = useDisclosure();
  const { register, handleSubmit, formState: { errors } } = useForm({resolver: yupResolver(createOusSchema),});
  
  const onInsertOu = async (FormData: any) => {
    ...
    if (token) {
      const result = await dataOus.post(registro, token);
      if(result === undefined){
        setData(null)
      }else{
        setData(result);
  ...
  return (
    <>
      ...
      >
        ...
        >
          {(onClose) => (
            <form onSubmit={ handleSubmit(onInsertOu) } noValidate>
